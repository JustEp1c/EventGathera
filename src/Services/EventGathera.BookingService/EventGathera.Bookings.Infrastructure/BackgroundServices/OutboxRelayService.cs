...
namespace EventGathera.Bookings.Infrastructure.BackgroundServices;
...
public class OutboxRelayService : BackgroundService
{
        ...
        CancellationToken ct)
    {
        ...
        {
            case "BookingCreated":
                var created = JsonSerializer.Deserialize<BookingCreated>(message.Payload);
                if (created != null)
                {
                    await eventPublisher.PublishBookingCreatedAsync(created, ct);
                }
                break;
            ...
            case "BookingConfirmed":
                ...
                if (confirmed != null)
                {
                    await eventPublisher.PublishBookingConfirmedAsync(confirmed, ct);
                }
                break;

            case "BookingRejected":
                var rejected = JsonSerializer.Deserialize<BookingRejected>(message.Payload);
                if (rejected != null)
                {
                    await eventPublisher.PublishBookingRejectedAsync(rejected, ct);
                }
                break;

            case "BookingCancelled":
                var cancelled = JsonSerializer.Deserialize<BookingCancelled>(message.Payload);
                if (cancelled != null)
                {
                    await eventPublisher.PublishBookingCancelledAsync(cancelled, ct);
                }
                break;
