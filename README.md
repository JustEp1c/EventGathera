# EventGathera
Проектная работа, выполняемая в ходе прохождения курса "Продвинутая разработка на C# и .NET" от Яндекс.Практикум.

## Стек технологий

- .NET 10
- ASP.NET Core Web API
- Entity Framework Core
- Apache Kafka (асинхронное взаимодействие)
- PostgreSQL
- Swagger/OpenAPI
- xUnit для тестирования
- Redis для кеширования
- Open Telemetry: Prometheus, Jaeger, Grafana

Перед запуском убедитесь, что установлены:

- .NET SDK 10
- PostgreSQL 16 или выше
- IDE или редактор кода
- Docker


## Архитектура системы

Система представляет собой **три независимых микросервиса**, каждый со своей базой данных и зоной ответственности. Сервисы общаются асинхронно через **Apache Kafka**, что обеспечивает слабую связанность и отказоустойчивость.

### Состав системы

| Сервис | Порт | База данных | Назначение |
|--------|------|-------------|------------|
| **Users Service** | 5000 | `users_db` | Регистрация, аутентификация, выдача JWT-токенов |
| **Events Service** | 5001 | `events_db` | Управление событиями (CRUD) и учёт доступных мест |
| **Bookings Service** | 5002 | `bookings_db` | Создание и отмена броней |

### Инфраструктура

| Компонент | Порт | Назначение |
|-----------|------|------------|
| **Kafka** | 9092 | Брокер сообщений для асинхронного обмена |
| **Zookeeper** | 2181 | Координация Kafka |
| **PostgreSQL** | 5432-5434 | Три отдельных экземпляра (по одному на сервис) |
| **Redis** | 6379 | Кеширование данных в сервисе событий (Events Service) |

---

## Поток данных при бронировании

### Схема взаимодействия

```text
┌─────────────┐     ┌─────────────────────┐     ┌─────────────────────┐
│   Клиент    │     │  Bookings Service   │     │  Events Service     │
│             │     │    (Порт 5002)      │     │    (Порт 5001)      │
└──────┬──────┘     └──────────┬──────────┘     └──────────┬──────────┘
       │                       │                           │
       │ POST /events/{id}/book│                           │
       │──────────────────────>│                           │
       │                       │                           │
       │                       │ 1. Создаёт бронь          │
       │                       │    (статус Pending)       │
       │                       │                           │
       │                       │ 2. Публикует событие      │
       │                       │    BookingCreated         │
       │                       │──────────────────────────>│
       │                       │    (топик: booking-created)│
       │                       │                           │
       │  202 Accepted         │                           │
       │<──────────────────────│                           │
       │                       │                           │
       │                       │                           │ 3. Валидирует событие:
       │                       │                           │    - существует ли
       │                       │                           │    - не началось ли
       │                       │                           │    - есть ли места
       │                       │                           │
       │                       │                           │ 4. Уменьшает
       │                       │                           │    AvailableSeats
       │                       │                           │
       │                       │ 5. Публикует ответ        │
       │                       │<──────────────────────────│
       │                       │    EventSeatReserved      │
       │                       │    (топик: event-seat-reserved)
       │                       │                           │
       │                       │    ИЛИ                     │
       │                       │<──────────────────────────│
       │                       │    EventSeatUnavailable   │
       │                       │    (топик: event-seat-unavailable)
       │                       │                           │
       │                       │ 6. Обновляет статус       │
       │                       │    брони: Confirmed /     │
       │                       │    Rejected               │
       │                       │                           │
       │ GET /bookings/{id}   │                           │
       │──────────────────────>│                           │
       │                       │                           │
       │   Актуальный статус  │                           │
       │<──────────────────────│                           │
       │                       │                           │
```

### Топики Kafka

| Топик | Издатель | Подписчик | Назначение |
|-------|----------|-----------|------------|
| `booking-created` | **Bookings Service** | **Events Service** | Уведомление о создании новой брони. Содержит идентификаторы брони, события и пользователя |
| `event-seat-reserved` | **Events Service** | **Bookings Service** | Подтверждение резервации мест. Содержит количество зарезервированных мест и обновлённое количество доступных мест |
| `event-seat-unavailable` | **Events Service** | **Bookings Service** | Отказ в резервации с указанием причины (событие не найдено, нет мест, событие уже началось) |
| `booking-confirmed` | **Bookings Service** | **Events Service** (опционально) | Бронь успешно подтверждена. Пока не используется |
| `booking-rejected` | **Bookings Service** | **Events Service** (опционально) | Бронь отклонена с указанием причины. Пока не используется |

### Детальный поток

1. Создание брони (Bookings Service)

- Пользователь отправляет POST /events/{id}/book

- Сервис создаёт бронь со статусом Pending

- Сохраняет в свою базу данных

- Публикует событие BookingCreated в Kafka

2. Валидация (Events Service)

- Получает BookingCreated из Kafka

- Проверяет существование события

- Проверяет, не началось ли событие

- Проверяет наличие свободных мест

- Если всё ОК: уменьшает AvailableSeats и публикует EventSeatReserved

- Если ошибка: публикует EventSeatUnavailable с причиной

3. Обновление статуса (Bookings Service)

- Получает EventSeatReserved или EventSeatUnavailable

- Обновляет статус брони (Confirmed или Rejected)

- Сохраняет изменения в своей базе

## Структура проекта

Проект построен на основе чистой архитектуры (Clean Architecture), сервисы делятся на четыре основных слоя:

```text
EventGathera/
├── src/
│   ├── shared/
│   │   └── EventGathera.Shared/          # Общие контракты и топики
│   │
│   └── services/
│       ├── EventGathera.UsersService/    # Сервис пользователей
│       │   ├── Domain/
│       │   ├── Application/
│       │   ├── Infrastructure/
│       │   └── Presentation/
│       │
│       ├── EventGathera.EventsService/   # Сервис событий
│       │   ├── Domain/
│       │   ├── Application/
│       │   ├── Infrastructure/
│       │   └── Presentation/
│       │
│       └── EventGathera.BookingsService/ # Сервис броней
│           ├── Domain/
│           ├── Application/
│           ├── Infrastructure/
│           └── Presentation/
│
├── tests/
├── docker-compose.yml
└── README.md
```

## Стратегия кеширования

В системе реализовано кеширование данных на уровне сервиса событий (Events Service) с использованием **Redis**. Кеширование построено на основе **стратегии инвалидации (Cache-Aside)**, что обеспечивает баланс между производительностью и актуальностью данных.

### Что кешируется

| Тип данных | Ключ | TTL | Описание |
|------------|------|-----|----------|
| **Отдельное событие** | `event:{EventId}` | 5 минут | Кешируется результат `GetEventByIdAsync` для быстрого доступа |
| **Топ-10 событий** | `events:top10` | 10 минут | Кешируется результат `GetTopEventsAsync` для быстрого отображения популярных событий |

### Стратегия Cache-Aside

```text
┌─────────────────────────────────────────────────────────────────────────────┐
│  Чтение данных (GetEventByIdAsync, GetTopEventsAsync)                       │
│                                                                             │
│  1. Проверить кеш → Если есть → вернуть данные                              │
│  2. Если нет → запросить из БД                                              │
│  3. Сохранить результат в кеш                                               │
│  4. Вернуть данные                                                          │
└─────────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────┐
│  Запись данных                                                              │
│                                                                             │
│  1. Обновить данные в БД                                                    │
│  2. Инвалидировать кеш (удалить ключ)                                       │
│  3. Следующий GET запрос получит свежие данные из БД и обновит кеш          │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Инвалидация кеша

При изменении данных кеш инвалидируется (удаляется), а не обновляется напрямую. Это гарантирует, что:

- Данные в кеше всегда соответствуют данным в БД

- Избегается проблема "гонки" между чтением и записью

- При следующем запросе данные будут прочитаны из БД и закешированы заново

### Обоснование выбора TTL

| TTL | Значение | Обоснование |
|-----|----------|-------------|
| **5 минут** (отдельное событие) | Умеренный срок | События могут меняться нечасто, а при каждом обновлении мы явно инвалидируем кеш. 5 минут — хороший баланс между актуальностью и производительностью |
| **10 минут** (топ-10 событий) | Увеличенный срок | Топ-10 — это агрегированный рейтинговый список, который меняется относительно редко. Небольшое устаревание здесь некритично — если места продаются, топ остаётся стабильным на протяжении нескольких минут. Явная инвалидация при каждом бронировании была бы избыточной и создавала бы лишнюю нагрузку на Redis |


## Наблюдаемость (Observability)

В системе реализован полноценный стек наблюдаемости, включающий сбор метрик, трассировку и визуализацию. Это позволяет отслеживать состояние сервисов, производительность и диагностировать проблемы.

### Инструменты наблюдаемости

| Инструмент | Назначение | Порт UI | Порт данных | URL |
|------------|------------|---------|-------------|-----|
| **Prometheus** | Сбор и хранение метрик | 9090 | 9090 | http://localhost:9090 |
| **Jaeger** | UI для просмотра трейсов | 16686 | 4317 (OTLP) | http://localhost:16686 |
| **Grafana** | Визуализация метрик и дашборды | 3000 | 3000 | http://localhost:3000 |

### Компоненты

#### Prometheus (Метрики)

Prometheus собирает метрики со всех сервисов, включая:

- **HTTP метрики**: количество запросов, длительность, статусы ответов
- **Runtime метрики**: использование памяти, GC, количество потоков

#### Jaeger (Трассировка)

Jaeger позволяет отслеживать путь запроса через всю систему:

- Время выполнения каждого запроса
- Зависимости между сервисами
- Ошибки и задержки в распределённых вызовах

#### Grafana (Визуализация)

Grafana предоставляет дашборды для мониторинга:

- Общая статистика по сервисам
- Графики нагрузки
- Алертинг (настраивается)

### Запуск стека наблюдаемости

Стек наблюдаемости запускается вместе со всей системой через `docker-compose`:

```bash
# Запуск всей системы с наблюдаемостью
docker-compose up -d

# Запуск только компонентов наблюдаемости
docker-compose up -d prometheus jaeger grafana
```

### Описание слоев

1. Domain (EventGathera.Domain)
Назначение: Содержит бизнес-сущности и правила предметной области.

Содержит:

Доменные сущности (Entity) - например, Event, Booking

Перечисления (Enum) - статусы, типы и т.д.

Доменные исключения - бизнес-правила, например:

ResourceNotFoundException - ресурс не найден

NoAvailableSeatsException - нет свободных мест

2. Application (EventGathera.Application)
Назначение: Содержит бизнес-логику приложения и Use Cases (варианты использования).

Содержит:

Интерфейсы сервисов (IEventService, IBookingService) - описывают Use Cases

Реализации сервисов (EventService, BookingService) - бизнес-логика

DTO (Data Transfer Objects) - для передачи данных между слоями:

Запросы: EventRequest, EventQueryParams

Ответы: EventResponse, PaginatedResult<T>

Интерфейсы репозиториев (IEventRepository, IBookingRepository) - порты для доступа к данным

Интерфейсы внешних сервисов - абстракции для внешних систем

Валидация - через IValidatableObject или FluentValidation

3. Infrastructure (EventGathera.Infrastructure)
Назначение: Реализует технические аспекты и адаптеры к внешним системам.

Содержит:

Реализации репозиториев - работа с базой данных через EF Core

DbContext и конфигурации - маппинг сущностей (EventConfiguration, BookingConfiguration)

Миграции - управление схемой базы данных

Реализации внешних сервисов - например, email-сервисы, API клиенты

Фоновые сервисы - фоновые задачи (BackgroundService)

4. Presentation (EventGathera.Presentation)
Назначение: Точка входа в приложение, взаимодействие с пользователем через HTTP API.

Содержит:

Контроллеры - обработка HTTP запросов

Глобальная обработка ошибок - маппинг доменных исключений в HTTP статусы

Swagger/OpenAPI - документация API

Composition Root - настройка DI контейнера в Program.cs

## Инструкция для запуска проекта

## Запуск через Docker Compose

```bash
# 1. Клонирование репозитория
git clone https://github.com/JustEp1c/EventGathera.git
cd EventGathera

# 2. Запуск всей системы
docker-compose up -d

# 3. Проверка статуса
docker-compose ps

# 4. Просмотр логов
docker-compose logs -f
```

Доступ к сервисам после запуска (Сервис	- Swagger UI)

Users Service	http://localhost:5000/swagger
Events Service	http://localhost:5001/swagger
Bookings Service	http://localhost:5002/swagger

## Локальный запуск (без Docker)

```bash
# 1. Убедитесь, что PostgreSQL и Kafka запущены
docker-compose up -d postgres kafka

# 2. Запуск сервисов по отдельности (в разных терминалах)

# Users Service
dotnet run --project src/services/EventGathera.UsersService/EventGathera.Users.Presentation

# Events Service
dotnet run --project src/services/EventGathera.EventsService/EventGathera.Events.Presentation

# Bookings Service
dotnet run --project src/services/EventGathera.BookingsService/EventGathera.Bookings.Presentation
```

### Запуск конкретного сервиса

```bash
# Пересобрать и перезапустить конкретный сервис
docker-compose up -d --build bookings-service

# Перезапустить сервис
docker-compose restart events-service

# Посмотреть логи конкретного сервиса
docker-compose logs -f users-servicez`
```

## Инструкция по запуску тестов

Запуск всех тестов

```bash
dotnet test
```

Запуск тестов с подробным выводом

```bash
dotnet test --verbosity detailed
```

---

## Возможности

Приложение поддерживает базовые CRUD-операции:

- создание события
- получение списка событий
- получение топ-10 событий (по популяности бронирований)
- получение события по идентификатору
- обновление события
- удаление события
- создание брони
- получение брони по идентификатору

### Фильтрация и пагинация событий

- **Фильтрация по названию** - поиск событий по частичному совпадению в названии (регистронезависимый)
- **Фильтрация по дате начала** - получение событий, начинающихся после указанной даты
- **Фильтрация по дате окончания** - получение событий, заканчивающихся до указанной даты
- **Комбинированная фильтрация** - одновременное применение нескольких фильтров
- **Пагинация** - разбивка результатов на страницы с настраиваемым размером

## Модели данных

### Event (Событие)

| Поле | Тип | Описание |
|------|-----|----------|
| Id | Guid | Уникальный идентификатор события |
| Title | string | Название события |
| Description | string | Описание события |
| StartAt | DateTime | Дата и время начала |
| EndAt | DateTime | Дата и время окончания |
| TotalSeats | int | Общее количество мест на событии |
| AvailableSeats | int | Текущее количество свободных мест|

### Booking (Бронь)

| Поле | Тип | Описание |
|------|-----|----------|
| Id | Guid | Уникальный идентификатор брони |
| EventId | Guid | ID события, на которое бронируют |
| Status | BookingStatus | Текущий статус брони |
| CreatedAt | DateTime | Дата и время создания брони |
| ProcessedAt | DateTime? | Дата и время обработки (для Confirmed/Rejected) |

### Статусы бронирования (BookingStatus)

| Статус | Значение | Описание |
|--------|----------|----------|
| Pending | 0 | Ожидает обработки (начальный статус) |
| Confirmed | 1 | Подтверждена (после успешной обработки) |
| Rejected | 2 | Отклонена (по различным причинам) |

---

## API Endpoints

### Бронирования (Bookings)

#### GET /bookings/{bookingId}

Получить бронь по ID.

**Пример запроса:**
```http
GET /bookings/987fcdeb-51a2-43d3-a456-426614174000
```
Пример ответа (200 OK):
```json
{
  "id": "987fcdeb-51a2-43d3-a456-426614174000",
  "eventId": "123e4567-e89b-12d3-a456-426614174000",
  "status": 1,
  "createdAt": "2026-04-14T10:30:00Z",
  "processedAt": "2026-04-14T10:30:05Z"
}
```

#### POST /events/{id}/book

Создать бронь на событие. Бронирование обрабатывается асинхронно в фоновом сервисе.

**Возможные статусы ответа:**
- `202 Accepted` - бронь успешно создана и ожидает обработки
- `404 Not Found` - событие с указанным ID не найдено
- `409 Conflict` - на событии нет свободных мест (овербукинг)

```http
POST /events/123e4567-e89b-12d3-a456-426614174000/book
```
Пример ответа (202 Accepted):
```json
{
  "id": "987fcdeb-51a2-43d3-a456-426614174000",
  "eventId": "123e4567-e89b-12d3-a456-426614174000",
  "status": 0
}
```

Пример ответа (409 Conflict):
```json
{
  "type": "https://tools.ietf.org/html/rfc9110#section-15.5.10",
  "title": "Conflict",
  "status": 409,
  "detail": "Нет свободных мест на это событие",
  "instance": "/events/123e4567-e89b-12d3-a456-426614174000/book"
}
```
---

## Фоновая обработка бронирований
### Как это работает

1. Создание брони: При POST запросе на /events/{id}/book создается бронь со статусом Pending

2. Фоновая обработка: BookingProcessingService (BackgroundService) запускается автоматически при старте приложения и работает в фоне:

* Каждые 2 секунды проверяет наличие новых броней

* Находит все брони со статусом Pending

* Обрабатывает каждую бронь (имитация обработки занимает 5 секунд)

* После успешной обработки отправляет сообщение BookingCreated

3. Получение статуса: Клиент может GET запросом на /api/bookings/{id} проверить текущий статус брони

```text
Клиент → POST /events/{id}/book → Бронь создана (Pending)
                                      ↓
                            BackgroundService (каждые 2 сек)
                                      ↓
                            Обработка брони (5 сек)
                                      ↓
                            Статус изменен на Confirmed
                                      ↓
Клиент ← GET /api/bookings/{id} ← Получен актуальный статус
```

## Синхронизация и потокобезопасность

Для обеспечения корректной работы в многопоточной среде используются следующие примитивы синхронизации:

1. SemaphoreSlim в BookingService

```text
private readonly SemaphoreSlim BookingLock = new(1, 1);

await _processingSemaphore.WaitAsync();
try
{
    ...Создание брони
}
finally
{
    _processingSemaphore.Release();
}
```

Назначение: Обеспечивает атомарность операций при создании бронирования. Блокировка гарантирует, что:

* Проверка наличия мест и их резервирование происходят как единая неделимая операция

* Два параллельных запроса не смогут одновременно проверить наличие мест и создать бронирование, превышающее лимит

* Вставка нового бронирования происходит потокобезопасно

## Сценарий с овербукингом

Рассмотрим ситуацию, когда на событие с 5 местами одновременно приходит 20 запросов на бронирование:

```text
Время 0: Событие имеет 5 свободных мест
Время 1: 20 пользователей одновременно нажимают "Забронировать"
         
         ┌─────────────────────────────────┐
         │  20 параллельных запросов       │
         │  POST /events/{id}/book         │
         └─────────────────────────────────┘
                      ↓
         ┌─────────────────────────────────┐
         │  lock (_bookingLock)            │
         │  Проверка и резервирование мест │
         └─────────────────────────────────┘
                      ↓
         ┌─────────────────────────────────┐
         │  5 успешных броней (Pending)    │
         │  15 ошибок 409 Conflict         │
         └─────────────────────────────────┘

Время 2: BackgroundService начинает обработку
         ┌─────────────────────────────────┐
         │  SemaphoreSlim (1 поток)        │
         │  Обработка 5 броней по очереди  │
         └─────────────────────────────────┘
                      ↓
         ┌─────────────────────────────────┐
         │  Все 5 броней → Confirmed       │
         │  AvailableSeats = 0             │
         └─────────────────────────────────┘
```

Результат:

* 5 пользователей успешно забронировали места (статус Pending → Confirmed)

* 15 пользователей получили ошибку 409 Conflict с сообщением "Нет свободных мест на это событие"

* У всех 5 успешных броней уникальные идентификаторы

* Событие имеет 0 свободных мест (AvailableSeats = 0)

Защита от овербукинга обеспечивается за счет:

1. Атомарной операции резервирования мест через lock

2. Проверки доступных мест перед созданием каждой брони

3. Синхронизации фоновой обработки через SemaphoreSlim

## Требования к базе данных

Для запуска приложения требуется **PostgreSQL**. Приложение использует Entity Framework Core для работы с базой данных.

### Настройка строки подключения

Строка подключения настраивается в файле `appsettings.json`:

```json
{
  "ConnectionStrings": {
    "DefaultConnection": "Host=localhost;Port=5432;Database=eventapi;Username=postgres;Password=postgres"
  }
}
```

Для локальной разработки вы можете использовать Docker:

```bash
docker run -d \
  --name eventapi-postgres \
  -e POSTGRES_USER=postgres \
  -e POSTGRES_PASSWORD=postgres \
  -e POSTGRES_DB=eventapi \
  -p 5432:5432 \
  postgres:16-alpine
```

Или используйте docker-compose.yml из репозитория:

```bash
docker-compose up -d
```

Схема базы данных создаётся автоматически при первом запуске приложения с использованием Migrate(). Вам не нужно выполнять миграции вручную.

Приложение автоматически:

1. Проверяет наличие базы данных

2. Создаёт базу данных, если она не существует

3. Создаёт все необходимые таблицы

4. Настраивает связи между таблицами


## Управление схемой базы данных

Схема базы данных управляется с помощью **Entity Framework Core Migrations**. Это позволяет отслеживать изменения в модели данных и применять их к базе данных контролируемым образом.

### Команды для работы с миграциями

#### Создание новой миграции

После внесения изменений в модель данных (добавление/изменение/удаление сущностей) необходимо создать новую миграцию:

```bash
dotnet ef migrations add <Название_миграции> --project ./src/EventGathera.Infrastructure --startup-project ./src/EventGathera.Presentation
```

#### Применение миграций к базе данных

Для применения всех ожидающих миграций к базе данных(необязательно, так как при запуске приложения срабатывает автоматический мигратор):

```bash
dotnet ef database update --project ./src/EventGathera.Infrastructure --startup-project ./src/EventGathera.Presentation
```

#### Откат к предыдущей миграции

Для отката к конкретной миграции:

```bash
dotnet ef database update <Название_предыдущей_миграции> --project ./src/EventGathera.Infrastructure --startup-project ./src/EventGathera.Presentation
```

#### Удаление последней миграции

Если миграция создана ошибочно и не была применена:

```bash
dotnet ef migrations remove --project ./src/EventGathera.Infrastructure --startup-project ./src/EventGathera.Presentation
```

#### Просмотр списка миграций

Для просмотра всех созданных миграций:

```bash
dotnet ef migrations list --project ./src/EventGathera.Infrastructure --startup-project ./src/EventGathera.Presentation
```

В проекте используются InMemory-провайдер Entity Framework Core для тестирования. Это позволяет запускать тесты без необходимости подключения к реальной базе данных.
Также присутствует интеграционное тестирование.

# Интеграционные тесты

В проекте реализованы интеграционные тесты для проверки взаимодействия с реальной базой данных. Тесты используют **PostgreSQL** внутри **Docker-контейнеров** с помощью библиотеки **Testcontainers**.

### Требования для запуска интеграционных тестов

Для запуска интеграционных тестов необходимо:

1. **Установленный Docker** (Docker Desktop или Docker Engine)
2. **Docker должен быть запущен** перед выполнением тестов
3. **Доступ к интернету** для скачивания образа PostgreSQL (при первом запуске)

### Как работают интеграционные тесты

При запуске интеграционных тестов происходит следующее:

1. **Создание контейнера**: Testcontainers создаёт временный Docker-контейнер с PostgreSQL
2. **Создание схемы**: В контейнере создаётся схема базы данных через `MigrateAsyn()`
3. **Выполнение тестов**: Тесты выполняются с использованием изолированной базы данных
4. **Очистка**: После завершения всех тестов контейнер автоматически удаляется

Каждый тест запускается в отдельном изолированном экземпляре базы данных, что гарантирует чистоту данных и независимость тестов друг от друга.

### Запуск интеграционных тестов

```bash
# Запуск всех тестов (включая интеграционные)
dotnet test
```

## 🔐 Аутентификация и авторизация

### Ролевая модель

В приложении реализована ролевая модель доступа с двумя уровнями прав:

| Роль | Описание | Права |
|------|----------|-------|
| **User** | Обычный пользователь | • Просмотр списка событий<br>• Просмотр деталей события<br>• Создание бронирований<br>• Отмена своих бронирований<br>• Просмотр своих бронирований |
| **Admin** | Администратор системы | • Все права пользователя<br>• Создание событий<br>• Редактирование событий<br>• Удаление событий<br>• Отмена любых бронирований |

### Разграничение прав доступа

Доступ к эндпоинтам контролируется с помощью атрибутов авторизации:

```csharp
// Доступ только для администраторов
[Authorize(Roles = "Admin")]
[HttpDelete("{id}")]
public async Task<IActionResult> DeleteEvent(Guid id)

// Доступ для всех аутентифицированных пользователей
[Authorize]
[HttpPost("{id}/book")]
public async Task<IActionResult> CreateBooking(Guid id)

// Публичный доступ (без аутентификации)
[AllowAnonymous]
[HttpPost("auth/login")]
public async Task<IActionResult> Login(string login, string password)
```

### Получение JWT-токена через Swagger

Зарегистрируйте пользователя через эндпоинт регистрации:

```text
POST /users/auth/register
{
  "login": "admin",
  "password": "secure_password",
  "role": "Admin"
}
```

Получите JWT-токен через эндпоинт логина:

```text
POST /users/auth/login
{
  "login": "admin",
  "password": "secure_password"
}
```

В ответе вы получите JWT-токен:

```text
{
  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
}
```

Авторизуйтесь в Swagger:

 - Откройте Swagger UI по адресу /swagger

 - Нажмите кнопку "Authorize" (🔒) в правом верхнем углу

 - В поле "Value" введите токен в формате:

 ```text
Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
```

- Нажмите "Authorize"

После успешной авторизации все запросы к защищённым эндпоинтам будут автоматически содержать заголовок авторизации.

### Настройка JWT в конфигурации

JWT-параметры настраиваются в файле appsettings.json:

```json
{
  "JwtSettings": {
    "Secret": "ваш-секретный-ключ",
    "Issuer": "event-gathera",
    "Audience": "event-gathera",
    "ExpiryMinutes": 60
  }
}
```

### Рекомендации по безопасности

⚠️ Важно: В продакшн-среде никогда не используйте значения секрета по умолчанию!

Для разработки
Используйте User Secrets для хранения JWT-секрета:

```bash
# Инициализация User Secrets
dotnet user-secrets init

# Установка секрета
dotnet user-secrets set "JwtSettings:Secret" "ваш-супер-секретный-ключ-минимум-32-символа"
```

Для продакшена
Используйте один из следующих способов:

Переменные окружения:

```bash
export JwtSettings__Secret="ваш-секретный-ключ"
```

Azure Key Vault или AWS Secrets Manager:

```json
{
  "JwtSettings": {
    "Secret": "#{JwtSettings.Secret}#"  // Заменяется при деплое
  }
}
```

Docker Secrets:

```yaml
services:
  app:
    environment:
      - JwtSettings__Secret=/run/secrets/jwt_secret
    secrets:
      - jwt_secret
```